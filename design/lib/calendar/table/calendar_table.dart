...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  Widget _buildCell(DateTime day, DateTime focusedDay) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        Widget? rangeHighlight = (widget.calendarBuilders??defaultWeekNumber).rangeHighlightBuilder
        ...
        if (!isDisabled) {
          final events = widget.eventsList
                  ?.where((value) => dateComparator(value, day))
                  .toList() ??
              [];

          Widget? markerWidget =
          (widget.calendarBuilders??defaultWeekNumber).markerBuilder?.call(context, day);
  ...
  Widget _buildSingleMarker(DateTime day, double markerSize) {
    return (widget.calendarBuilders??defaultWeekNumber).singleMarkerBuilder?.call(context, day) ??
        Container(
          width: markerSize,
          height: markerSize,
